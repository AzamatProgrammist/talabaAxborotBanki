...
<body>
	...
	<div class="insertTalaba">
		<a href="insertTalaba2.php" class="btns">Talaba Qo'shish</a>
	</div>	
<?php 

	$stmt = $pdo->prepare("SELECT * FROM kurs1_2");
	$stmt->execute();
	if ($stmt->rowCount()>=0)
	 {
					 	...
					 	<div class="container">
							<div class="box">
								...
								<p><?php echo $row['fio']; ?></p>
							 		...
							 		<a onclick="return confirm('Siz <?php echo $row['fio'] ?>ni bazadan olib tashlamoqchimisiz')" href="deletekurs1_2.php?id=<?php echo $row['id'] ?>">O'chirish</a>
							 </div>
					 	</div>
				 	</div>
				<?php
